...
\begin{example}
 La fonction identitée peut etre définie par le lambda terme suivant :
  %
  \[ 
  \lambda x. x
  \]
...
On considère que deux termes sont alpha equivalents si et seulement si ils sont 
égaux au renommage de leur variables liées près. 
\begin{example}
  Voici deux termes alpha equivalents 
  \begin{align*}
...
\end{example}
Dans cet exemple il n'y a pas eu de problème quand au nom des variables mais il faut 
faire attention 
...
C'est à dire que l'application (t u) ne peut pas etre réduite a moins 
...
\subsubsection{substitution}
%pas de grande difficulté a implémenter la fonction de substitution 
...
une redex il faut alors substitutier le membre de gauche dans le membre de 
droite. 
...
comme nous l'avons vu dans la section précédente. Cependant nous avons choisis 
d'enrichir le noyeau de notre language en créant trois nouveaux constructeurs.
Cela permet de ne pas alourdir l'écriture de certains termes ainsi que 
d'améliorer la représentation et d'automatiser des opérations d'évaluations.

METTRE LE CODE QUI CORRESPONDENT AUX DIFFERENTES PARTIE DES FONCTIONS 
...
IL FAUT QUE JE LES IMPLEMENTES EN VITE FAIT DANS LE LAMBDA CALCUL NON TYPE
