...
Un langage de programmation est entièrement défini par son compilateur. En effet ce logiciel permet de traduire
des programmes écrits dans un langage de haut niveau en instructions pour le processeur.

Dans ce rapport il sera question de comprendre le typage, une des étapes de la chaîne de compilation.
...
très compliqué de trouver leur source car l'instruction fautive ne correspond pas exactement à celle écrite par le programmeur.
...
il nous faut donc vérifier la validité du programme. Afin d'effectuer cette vérification, nous allons implémenter un vérificateur
...
sur l'égalité \ref{egalite} la contruction \equal{2+2}{N}{4} est équivalente  au type  $(id\: Nat\: (2\: +\: 2)\:\: 4)$.
...
d'exécuter \equal{1+2}{N}{4} de la même façon. Cela nous retourne bien le booléen \fun{false} comme
attendu.
...
Voici donc le type que dont nous souhaitons trouver un terme:

$\forall A:* \:\forall\: m:Nat\: \forall\: xs:Vec\: A\: m\: \forall\: n:Nat\: \forall\: ys:Vec\: A\: n\: \rightarrow\: Vec\: A\: (n\: +\: m)$

Cette spécification nous demande donc de crée un terme prenant quatre arguments: \\
...
Tout d'abord nous avons quatre type $\forall$ ce qui nous force à commencer par un terme avec quatre abstractions
\fun{(lambda A (lambda m (lambda xs (lambda n (lambda ys (?))))))}

A l'aide du constructeur $\Hole$ le type checker nous indique qu'il attend un terme de type $Vec\:A\:(n\: +\: m)$
...
Il nous faut donc définir un prédicat permettant $P$ permettant de générée le type $Vec\:A\:(n\: +\: m)$
une fois appliqué.
...
Voici la preuve rédiger à l'aide de AGDA un langage implémentant les types dépendants:
\begin{lstlisting}
...
Jérémie Salvucci pour avoir accepté de répondre à mes questions.
