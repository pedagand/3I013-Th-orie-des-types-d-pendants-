...
Il nous faut maintenant determiné un terme correspondant à ce type. La règle d'introduction
du type $id$ est le terme $refl$. Le seul terme

Afin de vérifier cette preuve dans notre type checker il nous faut exectuer la fonction suivante:

\fun{lcheck [] (read ``(succ (succ (succ (succ zero))))'')}

\fun{(read ``(id (+ (succ (succ zero)) (succ (succ zero))) (succ (succ (succ (succ zero)))))'')}

La fonction \fun{read} permet de parser une chaine de charactère pour obtenir le terme correspondant.
...
Pour vérifier que l'implémentation de notre type checker se comporte correctement tentons
executer \equal{1+2}{N}{4} de la même façon. Cela nous retourne bien le booléens \fun{false} comme
attendue.
