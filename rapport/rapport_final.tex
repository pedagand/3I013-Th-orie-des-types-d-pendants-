...
\codefrom{untyped}{lambda}{evaluation_sig}
...
\item[$\bullet$] \boxed{\inferrule[(Ann)]{\Gamma\vdash T \ni t}
...
\item[$\bullet$] \boxed{\inferrule[(Var)]{x:T \in \Gamma} 
                               {\Gamma \vdash x \in T }}\\
...
\item[$\bullet$] \boxed{\inferrule[(App)]{\Gamma \vdash f \in A\rightarrow B \quad \Gamma\vdash A \ni s }
...
constructeurs \True\: et \False\: sont désormais des \emph{termes vérifiables}
et le constructeur \Ifte\: un \emph{terme synthétisable}
...
Afin de transformer une valeur \lstinline!VLam f! en
 \fun{Abs} il nous faut appliquer la fonction \emph{f} à une variable
...
comme par exemple les vecteurs de taille $n$.
\begin{example}  
...
d'exécuter \equal{1+2}{N}{4} de la même façon. Cela nous retourne bien le booléen \fun{false} ce qui signifie que 
     ...
     (lambda n
      (lambda ys
       (dfold A (lambda mp (lambda vp (Vec A (+ mp n))))
         m xs (lambda nf (lambda vecun (lambda a (lambda xsf (dcons a xsf))))) 
           ys))))))  
\end{lstlisting}
