...
\label{meta_theorie}
\todo{Il y a encore des emph pour écrire des maths}
...
\todo{Utiliser les macros intga, succs, zero, iter, etc.} \todo{stop aux cfracs}
...
\succs{} et \zero{} sont des termes \todo{vérifiables} étant donné que
...
\todo{Utiliser les macros true, false, ifte,...}
...
\todo{incorrect : il faut les $\ni$ et les $\in$ pour indiquer les
  phases}
%
\[
\begin{array}{c}
...
leur type dépend de la paire sur laquelle ils sont appliqués. La paire
un \emph{terme vérifiable} car chaque paire à un type statique.

Règle de typages des paires\todo{ il va falloir en dire un peu plus comme, par exemple, qui est synthétisable et pourquoi.}:  \todo{Non aux cfracs!} \todo{Utiliser les macros Pair, Fst et Snd}
\begin{align*}
  &\cfrac{\Gamma\vdash A\ni a \:\: \Gamma\vdash B\ni b}
...
\todo{Utiliser/définir des macros pour mentionner le code. Ne pas utiliser emph.}
...
\todo{finir cette phrase :} Etant donné que nous utilisons un language de programmation utilisant déja
...
gérer certains méchanismes\todo{lesquels ?}.
...
Voici donc le code correspondant \todo{correspondant à quoi ?} en Ocaml:
...
\begin{example}
  ...
  \lstinline!fun arg -> arg! ce qui \todo{relire :} appliqué à la traduction $v_u$ de
  $u$, retournant ainsi $v_u$ comme souhaité.
\end{example}

\todo{Attention à ne pas jeter du code en pâture au lecteur : il doit
  y avoir une cohérence entre le texte et le code.}
...
\todo{Introduire le bout de code qui suit, ou faire passer le texte avant le code}

\codefrom{typed}{lambda}{big_step_eval_exTm_var}

La fonction \emph{VFree} permet simplement à partir d'un nom de créer
une \emph{NFree} à l'intérieure d'un \emph{VNeutral} \todo{cela ne
  veut rien dire : donne le rôle de cette fonction, pas ce qu'elle fait.}. Nous avons besoins de crée des \fun{neutral} pour
tous les termes dont l'évaluation est terminé.  Cette opération
pouvant être utile assez régulièrement \todo{le lecteur se  fiche de ce détail} on décide d'implémenter une
fonction à part entière dont voici le code:
...
Pour évaluer l'application d'un coup, il est nécessaire de créer une
fonction externe \todo{qu'est-ce qu'une ``fonction externe'' ?}
\emph{vapp}.  Nous appelons donc cette fonction avec le couple formé
...
crée un nouveau terme neutre contenant la fonction suspendue. Voici
donc le code de la fonction \fun{vapp}:
...
\todo{Incompréhensible :} Comme nous l'avons vu précédement, nous avons créé une \fun{value} correspondant
...
Nous allons pouvoir \todo{c'est quoi ``ceci'' ?} exploiter ceci avec l'aide de
