...
calculatoire des type. En effet notre seul but était de vérifier que nos $\lambda$-termes
...
une règle d'introduction qui à partire des hypothèses de formuler une conclusion.
Chaque constructeur possède aussi une règle d'éliminiation qui permet d'éliminer le constructeurs dans les hypothèses afin de
formuler une nouvelle conclusion, ainsi qu'une règle de calcul.
...
\todo{c'est super moche il faut que je trouve une structure pour le présenter}
\begin{lstlisting}
  Logique ------------- lambda calcul
  Axiom ----------> variable
  regles dintroductions ----------> constructeurs 
  regles deliminations ----------> Elminateurs 
  Normalisation des preuves ------> normalisation  
\end{lstlisting}
...
En ajoutant de nouveaux termes dans notre $\lambda$-calcul à permis d'introduire de nouveaux constructeurs
de la logique dans notre système. Nous allons suivre la même motivation en introduisant les types dépendants.

Si nous souhaitons prouver une proposition c'est à dire un type, il nous faudra trouver
un terme accepter par notre vérificateur de type.
Ceci permet d'aider grandement la recherche de démonstration étant donné que notre logiciel
de vérificateur de type nous assiste dans la création de nos termes nottament grâce à une amélioration
que nous verrons dans la section \ref{ex_preuves}.
...
Etant donné que nos types sont désormais des termes et que l'on peut les normaliser il nous faut
d'abord parler de l'évaluation.
...
\end{array}\]
La spécification de ces évaluations est très classique cependant pour leur implémentation
nous allons utilisé une stratégie plus similaire à l'implémentation précédente \ref{red_forte_grand_pas}. 
...
Les deux nouvelles constructions \emph{Pi} et \emph{Star} sont des \emph{termes vérifiables} étant donné
que l'on possède toute l'information nécéssaire pour les vérifier. De manière générale les types seront 
des \emph{termes vériable}.

Si l'introduction d'un système bidirectionnel dans le $\lambda$-calcul pouvait ne pas etre indispensable, elle prend tout son 
sens avec les types dépendants.
Cependant nous ne pouvons pas parler de types dépendants sans un système bidirectionnel, ce sont dans les 
\emph{termes vérifiables} que nous allons définir la dépendances que nous allons utilisé dans les 
...
un contexte, un \emph{terme inférable} ainsi qu'une \emph{value}.

\begin{itemize}
  
\item[$\bullet$] \boxed{\cfrac{\Gamma,x:S\vdash T \ni t}
  {\Gamma \vdash \forall x:S.T \ni \lambda x. t}\mbox{(Abs)}}\\
  ...
  anonyme comme vu en section \ref{maj_eval}. La substitution s'effectue donc à l'aide d'une
  application de fonction d'Ocaml.
...
\item[$\bullet$] \boxed{\cfrac{\Gamma \vdash t \in T' \:\: T=T'}
  ...
  nos \emph{value} en \emph{termes vérifiables}. Le second problème est que le nom associé 
...
\item[$\bullet$] \boxed{\cfrac{\Gamma \vdash * \ni S\:\:\Gamma,x:S \vdash *\ni T}
  ...
  Désormais \emph{Pi} n'est pas sous forme normale étant donné qu'il est le terme
...
\item[$\bullet$] \boxed{\cfrac{\Gamma \vdash f \in \forall x:S.T \quad \Gamma\vdash S \ni s }
  ...
  comme dans la règle de l'abstraction, par application de fonction Ocaml.
...
Maintenant l'ensemble des arguments de iter sont des termes vériables car l'introduction d'un prédicat
...
Les règles d'évaluations n'ont pas changées depuis la dernière implémentation mis à part le fait que  
...
C'est ce que nous allons essayer de traduire dans notre système.

\todo{mettre une introduction de règles issues de la logique}
...
retourner le type que l'on souhaite vérifier. %a changer
