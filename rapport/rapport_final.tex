...
\begin{example} 
  ...
  Soit \Var{x} une variable libre. \todo{Cela manque d'explication : je ne comprends pas cet exemple, qu'est-ce que tu veux montrer ?}
  En théorie : 
...
\item[$\bullet$] \boxed{\boxed{\Gamma\vdash ex \in T}}\\
...
règles de typage présentes en figure \ref{fig:typage_vec}. \todo{Utilise les macros dans la figure (et dans le texte) !}
\begin{figure}
\[
\begin{array}{c}
...
du type $id$ est le terme \todo{Macro !} $refl$. Le seul terme permettant de vérifier ce type
est le terme $refl{4}$


Afin de vérifier cette preuve dans notre type checker il nous faut exécuter la fonction suivante: \todo{retour à la ligne dans le listing}
...
attendu. \todo{il faut expliquer le booléen false en question n'est pas le résultat de l'égalité mais du type checker !}
...
Ce qui nous donne comme terme final: \todo{saut à la ligne dans le listing}
