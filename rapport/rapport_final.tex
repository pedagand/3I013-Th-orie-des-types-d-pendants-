...
Un des problèmes introduit par cette représentation des variables est qu'il devient assez compliqué de déterminer si une variable est libre ou non, comme on peut le constater dans l'exemple ci-dessus (Exemple~\ref{example:ouvert-clos}). 
...
\begin{example}
  Soit le terme $\Lam{x}{\subst{x}{x}{\Lam{y}{y}}}$     \(\lambda\)x.(\subst{x}{x}{\(\lambda\)y.y}), après avoir effectué la
  substitution nous obtenons le terme $\Lam{x}{\Lam{y}{Var{y}}}$  \(\lambda\)x.\(\lambda\)y.
\end{example}
...
\begin{align*}
    Reduction(x) &\triangleq \Var{x} \\
    Reduction(\Lam{x}{t}) &\triangleq \Lam{x}{t} \\
    Reduction(\App{\Var{x}}{\Var{u}}) &\triangleq \App{\Var{x}}{\Var{u}} \\
    Reduction(\App{\Lam{x}{\Var{t}}}{\Var{u}}) &\triangleq \subst{t}{x}{u} \\
\end{align*}
...
\begin{align*}
  ...
  \Ifte &\triangleq \lambda x.\lambda y.\lambda z. x\: y\: z  \Lam{x}{\Lam{y}{\Lam{z}{\App{\Var{x}}{\App{\Var{y}}{\Var{z}}}}}}  \\
\end{align*}
...
Les applications dont le membre de gauche est une abstractions posent problème ~\ref{probleme:type_simple} 
