...
Les languages de programmations sont entièrements définis par leur compilateur.....
...
lambda, dans l'Exemple~\ref{example:id}, $x$ était une variable \emph{liée}.
...
Comme les abstractions lies les variables elles sont comunéments appelées des \emph{lieurs}
...
Le problème avec cette représentation est que le terme \Lam{\Var{x}}{\Var{x}} et \Lam{\Var{y}}{\Var{y}} sont considérés comme distincts 
à cause du nom de leurs variable, or ce n'est pas le cas. Il faut donc introduire la notion 
d'$\alpha$-équivalence.
\end{remark}
...
\begin{align*}
    ...
    \subst{\lambda y.t}{x}{u} &\triangleq \lambda x.\subst{t}{x}{u} \\
    \subst{\lambda x.t}{x}{u} &\triangleq \lambda x.t \\
    \subst{(t\: v)}{x}{u} &\triangleq \subst{t}{x}{u}\: \subst{v}{x}{u}
\end{align*}

\begin{example}
  Soit le terme $\subst{(\Lam{z}{x})}{x}{\Lam{y}{y}}$, après avoir effectué la
  substitution nous obtenons le terme $\Lam{z}{\Lam{y}{\Var{y}}}$.
\end{example}
...
L'argument \fun{var} va nous permettre de compter le nombre de \emph{lieurs} traversés.
Cette valeur correspondra donc à la valeur de la variable liée à substituer. C'est pour cette
raison que dans le cas où \fun{terme} est une variable liée d'indice \fun{x} on test l'égalité entre 
\fun{var} et \fun{x}.


\begin{example}
  Soit le terme t = \Lam{y}{\App{x}{y}} qui coresspond au terme \\
  Ocaml \fun{Abs(``y''Appl(BoundVar 1,BoundVar 0))}.
  Voici le déroulement de l'appel à la fonction \fun{substitution t 0 FreeVar''z''}: \\
  -\fun{Abs(``y''Appl(BoundVar 1,BoundVar 0))}$\rightarrow$ \fun{substitution Appl(BoundVar 1,BoundVar 0) 1 FreeVar''z''} \\
  -\fun{Appl(BoundVar 1,BoundVar 0)}$\rightarrow$ \fun{substitution (BoundVar 1) 1 FreeVar''z''} \\
  -\fun{BoundVar 1} $\rightarrow$ \fun{FreeVar ``z''} \\ 
  -\fun{BoundVar 0} $\rightarrow$ \fun{BoundVar 0} \\
  Cela nous renvoie donc le terme  \fun{Abs(``y''Appl(FreeVar ``z'',BoundVar 0))}
\end{example}

\begin{proposition}
  
\end{proposition}
Grace à notre représentation locally nameless il n'y à pas besoin de faire d'$\alpha$-convertions lors de la
substitution. Dans le cas ou une des variables à substituer à été trouvé, on retourne directement le terme substituant.
...
\emph{normal order strategy}, \emph{l'appel par valeur}.
Chacune des stratégie d'évaluation possède des avantages et
des inconvénients. Dans le $\lambda$-calcul non typé nous avons
choisis d'utiliser \emph{l'appel par nom} car c'est un très bon 
compromis, celle ci obtient toujours la forme normale si elle 
existe. 
...
\todo{roman: rajouter les paires dans le prog}
...
\todo{Définir des macros pour les jugements de typage, synthèse de
  type et vérification de type}.
\todo{On ne doit pas voir apparaître le terme ``inférence de type'' dans ce rapport : attention !}
...
\todo{Introduire ces définitions :} 
...
\todo{ne pas lister mécaniquement les arguments : donner une
  spécification informelle de la fonction en nommant les arguments (et
  leur type) en passant} ayant pour paramètres: un contexte qui est
une liste de paires de la forme \emph{(var,type)}, un \emph{inTerme}
ainsi qu'un type. Elle retourne un booléen indiquant
si le terme est correctement typé ou non. \label{check}
%
...
\attention{Ce n'est pas la raison : c'est juste que l'on va sous des abstractions. Quand on va sous une abstraction, la variable autrefois liée devient libre et notre représentation nous impose donc de lui donner un nom explicite.} Afin de pouvoir associé une variable à son type dans le contexte il nous faut un nom pour celle ci.
...
\emph{exTerme} et renvoie le type de celui-ci.
...
Cependant il \attention{texte pas clair : tu veux dire que ce cas est impossible.} n'est pas possible de synthétiser une variable liée étant donné qu'elles sont libérés lors
...
\todo{Transférer le code dans \texttt{untyped} et mettre à jour le texte.}
...
l'on peut aisément vérifier qu'ils appartiennent au type des
entiers. \todo{Dire quelque chose sur \iter{}. Remarque : j'ai décidé
  de mettre le type de retour \Var{A} dans la définition de \iter{},
  pour être raccord avec ce que l'on fait avec les types
  dépendants. Et plus de question de ``pourquoi on synthétise ?''}.
...
Nous pouvons désormais typer nos booléens de la manière suivante: \todo{Stop les cfracs}
\begin{align*}
  &\cfrac{}
  {\Gamma\vdash Bool \ni true}&\mbox{(True)} \\
  &\cfrac{}
  ...
  {\Gamma\vdash \mathrm{ifte}\: C\: c\: t\: e\in C}&\mbox{(Ifte)}
\end{align*}

\subsubsection{Le produit cartésien}
\label{produit_cartesien}

\todo{Extraire la partie syntaxe \& réduction et l'amener en Section~\ref{paires}}
\todo{Définir des commandes pour écrire les paires et les 2 projections}
...
introduisons donc un type \emph{produit} $A \times B$. \todo{Étendre formellement la grammaire des types \Var{T}}

Nous allons maintenant évaluer nos termes, \todo{en dire plus}
\begin{align*}
...
Règle de typages des paires: \todo{en dire plus.}
\begin{align*}
  &\cfrac{\Gamma\vdash A\ni a \:\: \Gamma\vdash B\ni b}
...
notre language. Tout d'abord, il nous faut introduire trois nouveaux
termes ainsi qu'un nouveau constructeurs de type. \todo{c'est (trop !) brut de
  décoffrage !}
\emph{Type}
...
Rien à commenter, \todo{ha bon ? Très bien alors}
...
la réduction forte qui exploite le language de programmation
fonctionnelle que l'on utilise pour \emph{implémenter} la réduction
forte.  \attention{Ceci n'est pas clair : tu es en train de donner une
  semi-définition de ``normaliser''. Je préférerai que tu développes
  l'aspect ``hack'' où l'on exploite les fonctions de OCaml pour
  modéliser les fonctions de notre langage. Idée clé : on implémente
  un interpréteur pour nos termes en utilisant les termes OCaml pour
  interpréter les fonctions de notre langage} Elle a aussi pour effet
d'évaluer complètement les termes, on peut le constater du fait \if 0
a changer \fi que les fonctions prenenent des \emph{inTermes} ou des
\emph{exTermes} en entrée et retourne des valeurs.  Les valeurs ne
sont plus réinterprétées par la suite.
...
Voici donc le code correspondant en Ocaml: \todo{Il va falloir m'en
  dire \emph{beaucoup} plus.}
%
\codefrom{typed}{lambda}{value}
\codefrom{typed}{lambda}{neutral}

\todo{Définir et utiliser des macros pour les termes OCaml, comme VLam.}

\newcommand{\VLam}{\lstinline!VLam!}

La value \emph{VLam} est représentée par une fonction Ocaml \todo{je
  vois mais \emph{pourquoi} ?}.  Commençons par donner le code de
celle ci, nous le commenterons après\todo{Non, commente au fil de
  l'eau, c'est plus digeste}.

\todo{Commenter l'annotation}
\codefrom{typed}{lambda}{big_step_eval_exTm_ann}
\todo{Commenter la variable}
\codefrom{typed}{lambda}{big_step_eval_exTm_var}
\todo{Commenter l'application}
...
L'abstraction est interprétée comme une fonction anonyme Ocaml puis évaluons 
le terme sous le $\lambda$. \todo{étoffer}

\codefrom{typed}{lambda}{big_step_eval_inTm_abs}

%revoir ce paragraphe.
\todo{Donner cette explication plus tôt}
...
étant donné qu'elle est encodée par une fonction anonyme OCaml.

La fonction \emph{VFree} permet simplement à partir d'un nom de créer
une \emph{NFree} à l'intérieure d'un \emph{VNeutral}.  Elle permet
simplement d'alléger le code de la fonction. \todo{C'est du morse
  ça. Explique le ``pourquoi'', pas ``comment'' cette fonction opère
  (je peux lire le code, merci)}
...
$\lambda$-calcul. \todo{La phrase suivante ne dit rien de très
  utile. Il serait plus utile de décrire le type de la fonction (et
  donc ses arguments) et donner une intuition de son comportement.} Il
s'agit donc implémenter cette fonction permettant de faire cette
association, voici son code:

\codefrom{typed}{lambda}{value_to_inTm}
\codefrom{typed}{lambda}{neutral_to_exTm}

Le type \emph{name} nous permet de faire la distinction entre les
variables réellement libre et celles que nous souhaitons liée \todo{ce
  n'est pas le plus important : il faut expliquer que l'on est forcé
  d'introduire des variables artificielles pendant la réification, que
  l'on veut lier à leur abstraction (contrairemnt aux
  \lstinline!FreeVar!)}.  Afin de transformer une value \emph{VLam} en
...
possède un indice.  Nous allons pouvoir exploiter ceci avec l'aide de
