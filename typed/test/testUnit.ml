...
let test_check1 test_ctxt = assert_equal 
			      (check [] (Abs(Global "f",Abs(Global "g",Inv(Appl(BVar 1,Inv(BVar 0)))))) (Fleche(Fleche(Bool,Bool),Fleche(Bool,Bool)))) 
			      (true)
let test_check2 test_ctxt = assert_equal 
			      (check [] (Inv(Appl(Ann(Abs(Global "x",Inv(BVar 0)),Fleche(Fleche(Bool,Bool),Fleche(Bool,Bool))),(Abs(Global "y",(Inv(BVar 0))))))) (Fleche(Bool,Bool)) ) 
			      (true)
let test_check3 test_ctxt = assert_equal 
			      (check [] (Inv(Iter(Succ(Zero),Abs(Global "x",Inv(BVar 0)),Ann(Zero,Nat)))) Nat) 
			      (true) 
...
let test1_typed_to_simple test_ctxt = assert_equal 
			(typed_to_simple_inTm (Abs(Global "x",Inv(Appl(BVar 0,Inv(FVar (Global "0")))))))
			(SAbs(SAppl(SBVar 0,SFVar "0")))
let test2_typed_to_simple test_ctxt = assert_equal 
			(typed_to_simple_inTm (Inv(Appl(Ann(Abs(Global "x",Inv(BVar 0)),Fleche(Fleche(Bool,Bool),Fleche(Bool,Bool))),(Abs(Global "y",(Inv(BVar 0))))))))
...
let test_sub_inTm1 text_ctxt = assert_equal 
			 (substitution_inTm (Abs(Global "x",Inv(Appl(BVar 0,Inv(FVar (Global "0")))))) (FVar (Global "y")) (-1))
			 ((Abs(Global "x",Inv(Appl(FVar (Global "y"),Inv(FVar (Global "0")))))))

let test_sub_inTm2 test_ctxt = assert_equal
			(substitution_inTm (Abs(Global "x",Inv(Appl(BVar 0,Inv(FVar (Global "0")))))) (Ann(Abs(Global "y",Inv(BVar 0)),Fleche(Bool,Bool))) (-1))
			((Abs(Global "x",Inv(Appl((Ann(Abs(Global "y",Inv(BVar 0)),Fleche(Bool,Bool))),Inv(FVar (Global "0"))))))) 
let test_sub_inTm3 test_ctxt = assert_equal
				 (substitution_inTm (Inv(P0(Ann(Pair(Inv(BVar 0),Inv(BVar 0)),(Croix(Nat,Nat)))))) (FVar (Global "x")) 0)
				 (Inv(P0(Ann(Pair(Inv(FVar (Global "x")),Inv(FVar (Global "x"))),Croix(Nat,Nat)))))
let test_sub_inTm4 test_ctxt = assert_equal
				 (substitution_inTm  (Inv(Ifte(True,BVar 0,BVar 0))) (FVar (Global "x")) 0 )
				 (Inv(Ifte(True,FVar (Global "x"),FVar (Global "x")))) 


(* ------------------------test de pretty printing------------------------ *)

let test8 test_ctxt = assert_equal 
			(read(pretty_print_inTm (Inv(Appl(Ann(Abs(Global "x",Inv(BVar 0)),Nat),Inv(FVar (Global "y"))))) []))
			(read "((: (lambda x x) N) y)")
let test9 test_ctxt = assert_equal 
			(read(pretty_print_inTm (Inv(Iter((Succ(Zero)),(Abs(Global "x",Inv(BVar 0))),FVar (Global "y"))))  [] ))
			(read "(iter (succ zero) (lambda x x) y)")
let test10 test_ctxt = assert_equal 
			 (read(pretty_print_inTm (Inv(Appl(Ann(Abs(Global "x",Inv(BVar 0)),Fleche(Fleche(Bool,Bool),Fleche(Bool,Bool))),(Abs(Global "y",(Inv(BVar 0))))))) [] ))
			 (read "((: (lambda (x) (x)) (-> (-> B B) (-> B B))) (lambda (y) (y)))")
let test11 test_ctxt = assert_equal
			 (read(pretty_print_inTm (Abs(Global "x",Abs(Global "y",Abs(Global "z",Inv(Appl(BVar 2, Inv(Appl(BVar 1, Inv(BVar 0))))))))) [] ))
			 (read "(lambda (x y z) (x (y z)))")
let test12 test_ctxt = assert_equal
			 (read(pretty_print_inTm (Pair(Abs(Global "x",Inv(BVar 0)),Inv(FVar(Global "y")))) [] ))
			 (read "(, (lambda x x) (y))") 
let test13 test_ctxt = assert_equal 
			 (read(pretty_print_inTm (Inv(Ann(Abs(Global "x",Inv(BVar 0)),Croix(Bool,Bool)))) []))
			 (read "(: (lambda x x) (* B B))")
let test14 test_ctxt = assert_equal 
			 (read(pretty_print_inTm (Inv(Ifte(True,Ann((Pair(True,False)),(Croix(Bool,Bool))),FVar (Global "y"))))[] ))
